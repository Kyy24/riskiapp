...
public class SMA2PKU extends ListActivity {
    ...
    private void tampilkanpilhan(String pilhan) {
        try {
            ...
            {
                String lokasirs = "https://goo.gl/maps/WGHQfUfeThYMHeas5";
                a = new Intent(Intent.ACTION_VIEW,Uri.parse(lokasirs));

            } else if (pilhan.equals("Website"))
            {
                String web = "https://sman2pekanbaru.sch.id/";
                a = new Intent( Intent.ACTION_VIEW,Uri.parse(web));

            } else if (pilhan.equals("Info di Google"))
            {
                a = new Intent(Intent.ACTION_WEB_SEARCH);
                a.putExtra(SearchManager.QUERY,"SMA 2 PEKANBARU");
